...
public final class ReferenceMemoryBuffer extends Buffer {
	...
	ReferenceMemoryBuffer(final int bufferSize, final ByteBuffer byteBuffer, final MemoryBufferPoolConnector bufferPoolConnector) {
		...
		this.bufferRecycler = null; //  new MemoryBufferRecycler(byteBuffer, bufferPoolConnector);
	...
	private ReferenceMemoryBuffer(final int bufferSize, final ByteBuffer byteBuffer, final MemoryBufferRecycler bufferRecycler) {
	 ...
	 */
	@Override
	public ReferenceMemoryBuffer duplicate() {

		if (this.writeMode.get()) {
			throw new IllegalStateException("Cannot duplicate buffer that is still in write mode");
		}

		final ReferenceMemoryBuffer duplicatedMemoryBuffer = new ReferenceMemoryBuffer(this.byteBuffer.limit(),
